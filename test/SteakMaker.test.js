...
const SteakMaker = artifacts.require('SteakMaker');
const MockERC20 = artifacts.require('MockERC20');
const UniswapV2Pair = artifacts.require('UniswapV2Pair');
const UniswapV2Factory = artifacts.require('UniswapV2Factory');

contract('SteakMaker', ([alice, bar, minter]) => {
    beforeEach(async () => {
        this.factory = await UniswapV2Factory.new(alice, { from: alice });
        this.steak = await SteakToken.new({ from: alice });
        await this.steak.mint(minter, '100000000', { from: alice });
        ...
        this.maker = await SteakMaker.new(this.factory.address, bar, this.steak.address, this.weth.address);
        this.steakWETH = await UniswapV2Pair.at((await this.factory.createPair(this.weth.address, this.steak.address)).logs[0].args.pair);
    ...
    it('should make steaks successfully', async () => {
        await this.factory.setFeeTo(this.maker.address, { from: alice });
        await this.weth.transfer(this.steakWETH.address, '10000000', { from: minter });
        await this.steak.transfer(this.steakWETH.address, '10000000', { from: minter });
        await this.steakWETH.mint(minter);
        ...
        // After calling convert, bar should have steak value at ~1/6 of revenue
        await this.maker.convert(this.token1.address, this.token2.address);
        assert.equal((await this.steak.balanceOf(bar)).valueOf(), '32965');
        assert.equal((await this.token1Token2.balanceOf(this.maker.address)).valueOf(), '0');
        // Should also work for STEAK-ETH pair
        await this.steak.transfer(this.steakWETH.address, '100000', { from: minter });
        await this.weth.transfer(this.steakWETH.address, '100000', { from: minter });
        await this.steakWETH.sync();
        await this.steak.transfer(this.steakWETH.address, '10000000', { from: minter });
        await this.weth.transfer(this.steakWETH.address, '10000000', { from: minter });
        await this.steakWETH.mint(minter);
        assert.equal((await this.steakWETH.balanceOf(this.maker.address)).valueOf(), '16537');
        await this.maker.convert(this.steak.address, this.weth.address);
        assert.equal((await this.steak.balanceOf(bar)).valueOf(), '66249');
        assert.equal((await this.steakWETH.balanceOf(this.maker.address)).valueOf(), '0');
    });
});
